...
def _action(world, type="move_entity", target="p17", params=None, approved_by=None):
    action = Action(
        id=world.new_id("act"),
        type=type,
        actor="tester",
    ...
    if approved_by:
        action.authorization = {"approved_by": approved_by, "approved_at": world.now()}
    return action
...
def test_approval_check(observed_sim):
    world = observed_sim.world
    constraint = next(c for c in world.constraints if c.check == "approval_required")
    action = _action(world, params={"to_zone": "storage-b"})
    ok, reason = checks.CHECKS["approval_required"](action, world, constraint)
    assert ok is False and "approval" in reason
    action2 = _action(world, params={"to_zone": "storage-b"}, approved_by="rosario")
...
def test_boundary_check(observed_sim):
    world = observed_sim.world
    constraint = next(
        c for c in world.constraints if c.check == "zone_boundary_forbidden"
    )
    action = _action(world, params={"to_zone": "office"})
    ok, reason = checks.CHECKS["zone_boundary_forbidden"](action, world, constraint)
    assert ok is False and "office" in reason
    action2 = _action(world, params={"to_zone": "packing"})
    ok, _ = checks.CHECKS["zone_boundary_forbidden"](action2, world, constraint)
    assert ok is True
...
def test_unknown_check_fails_closed(observed_sim):
    ...
    world.constraints.append(
        Constraint(
            ...
            name="bogus",
            kind="safety",
            applies_to=["move_entity"],
            check="no_such_check",
        )
    )
    action = _action(world, params={"to_zone": "storage-b"}, approved_by="rosario")
    results = checks.evaluate(action, world)
    bogus = [
        r for c, r, _ in [(c, ok, rs) for c, ok, rs in results] if c.id == "c-bogus"
    ]
...
def test_verify_verified(observed_sim):
    sim = observed_sim
    action = sim.act(
        "move_entity", "tester", "p17", {"to_zone": "packing"}, approved_by="rosario"
    )
...
def test_verify_failed_on_contradiction(observed_sim):
    ...
    action = engine.propose(
        "move_entity", "tester", "p17", {"to_zone": "packing"}, provenance="test"
    )
...
def test_verify_unknown_without_observation(sim):
    ...
    action = engine.propose("move_entity", "tester", "p17", {"to_zone": "packing"})
    engine.authorize(action, approved_by="rosario")
