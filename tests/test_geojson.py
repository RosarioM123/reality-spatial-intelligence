...
def test_round_trip_preserves_world():
    ...
    assert [(p.x, p.y) for p in zone.polygon] == [(0, 0), (10, 0), (10, 10), (0, 10)]
...
def test_ring_is_closed_and_valid_geojson():
    ...
    zone_feat = next(f for f in fc["features"] if f["properties"]["kind"] == "zone")
    ...
    ent_feat = next(f for f in fc["features"] if f["properties"]["kind"] == "entity")
...
def test_rejects_duplicate_ids():
    ...
    with pytest.raises(GeoJSONError):
        geojson_to_world({"type": "FeatureCollection", "features": [feat, feat]})
...
def test_open_ring_is_closed_on_import():
    ...
    world = geojson_to_world({"type": "FeatureCollection", "features": [feat]})
