...
def test_full_lifecycle_success(observed_sim):
    sim = observed_sim
    action = sim.act("move_entity", "tester", "p17", {"to_zone": "storage-b"},
                     approved_by="rosario", provenance="test")
    ...
    types = [e.type for e in sim.world.events
             if e.caused_by and e.caused_by.get("id") == action.id]
    for expected in ("action_proposed", "action_authorized", "action_executed",
                     "action_observed", "action_verified"):
...
def test_hard_constraint_rejects(observed_sim):
    ...
    action = sim.engine.run("move_entity", "tester", "p17",
                            {"to_zone": "office"}, approved_by="rosario")
...
def test_unknown_zone_rejects(observed_sim):
    sim = observed_sim
    action = sim.engine.run("move_entity", "tester", "p17",
                            {"to_zone": "atlantis"}, approved_by="rosario")
    assert action.status == "rejected"
...
def test_capabilities_are_enforced(observed_sim):
    ...
    with pytest.raises(ValueError, match="not offered"):
        sim.engine.propose("assign_entity", "tester", "p17",
                           {"assignee": "ana"})
...
def test_reserve_unavailable_resource_rejected(observed_sim):
    sim = observed_sim
    first = sim.act("reserve_resource", "tester", "forklift-1",
                    {"for_whom": "ana"})
    assert first.status == "verified"
    # Already reserved: the second request is rejected by the constraint.
    second = sim.engine.run("reserve_resource", "tester", "forklift-1",
                            {"for_whom": "rosario"})
...
def test_reserve_available_resource(observed_sim):
    sim = observed_sim
    action = sim.act("reserve_resource", "tester", "forklift-1",
                     {"for_whom": "ana"})
    assert action.status == "verified"
...
def test_change_status_and_assign(observed_sim):
    sim = observed_sim
    a1 = sim.act("change_status", "tester", "dock-door-1",
                 {"status": "open"})
    assert a1.status == "verified"
    assert sim.world.get_attribute("dock-door-1", "status") == "open"
    a2 = sim.act("assign_entity", "tester", "forklift-1",
                 {"assignee": "ana"})
...
def test_notify_has_no_observable_effect(observed_sim):
    sim = observed_sim
    action = sim.act("notify", "tester", "ana",
                     {"message": "shift ends at 18:00"})
    assert action.status == "verified"
    assert action.verification["status"] == "verified"
...
def test_execute_requires_authorization(observed_sim):
    sim = observed_sim
    action = sim.engine.propose("change_status", "tester", "dock-door-1",
                                {"status": "open"})
