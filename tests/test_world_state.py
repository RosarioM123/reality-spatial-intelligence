...
def _obs(world, entity_id, state, ts=1000.0, source="test-cam",
         confidence=0.9, oid=None):
    return Observation(
...
def test_state_change_event_records_old_and_new(observed_sim):
    ...
    changes = [e for e in world.events[before:]
...
def test_location_update_via_observation(observed_sim):
    ...
    world.apply_observation(
        _obs(world, "p17", {"zone_id": "packing"}, ts=2000.0))
...
def test_no_event_when_nothing_changed(observed_sim):
    ...
    before = len(world.events)
    world.apply_observation(
        _obs(world, "p17", {"status": "intake"}, ts=2000.0))
...
def test_world_validation_catches_bad_relationship(observed_sim):
    ...
    world.relationships.append(
        Relationship("p17", "inside", "no-such-zone"))
