...
def test_full_e2e_loop_success(observed_sim):
    ...
    action = sim.act("move_entity", "ai-agent", "p17",
                     {"to_zone": "storage-b"},
                     approved_by="rosario", provenance="e2e")
    # 3. events were emitted for every transition
    types = {e.type for e in world.events}
    for expected in ("observation_recorded", "state_changed",
                     "action_proposed", "action_authorized", "action_executed",
...
def test_e2e_executor_lie_detected(observed_sim):
    ...
    world = sim.world
    engine = sim.engine
    action = engine.propose("move_entity", "ai-agent", "p17",
                            {"to_zone": "packing"}, provenance="e2e")
    ...
    reported = next(e for e in world.events
                    if e.type == "action_executed"
                    and e.caused_by.get("id") == action.id)
...
def test_rejected_action_leaves_no_belief_change(observed_sim):
    ...
    action = sim.engine.run("move_entity", "t", "p17", {"to_zone": "office"},
                            approved_by="rosario")
    assert action.status == "rejected"
    assert world.get_attribute("p17", "zone_id") == "storage-a"
    # The rejection itself is recorded; the world is not.
    rejected = [e for e in world.events[n_events:]
