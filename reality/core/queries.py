...
def _event_summary(world: RealityWorld, e) -> dict:
    return {
        "id": e.id, "ts": e.ts, "type": e.type, "entity_id": e.entity_id,
        "actor": e.actor, "details": e.details, "caused_by": e.caused_by,
    }


def _plan_workstation(
    world: RealityWorld, team_ref: str, person: str | None,
    runtime=None, approved_by: str | None = None,
) -> dict:
    ...
    if team is None or team.kind != "team":
        ...
        if team is None:
            return {"type": "not_found",
                    "message": f"no team matching {team_ref!r}"}
    origin = team.position
    candidates = [
        e for e in world.spatial.entities.values()
        if e.kind == "workstation"
        and e.state.get("availability") == "available"
    ]
    from reality.core import geometry as _g
    ranked = sorted(candidates,
                    key=lambda e: _g.distance(e.position, origin))
    if not ranked:
        return {"type": "no_workstation",
                "message": "no available workstation found"}
    pick = ranked[0]
    steps = [
        {"action": "reserve_resource", "target": pick.id,
         "parameters": {"for_whom": person or "unassigned"}},
        {"action": "assign_entity", "target": pick.id,
    ...
    if person and runtime is not None:
        ...
        for step in steps:
            if step["blocked"]:
                ...
                continue
            action = runtime.act(
                step["action"], actor="ask", target=step["target"],
                parameters=step["parameters"], provenance="ask",
                approved_by=approved_by)
            executed.append(action.to_dict())
        result["execution"] = executed
    elif person:
        result["execution"] = ("not executed: no runtime attached "
                               "(load the world in a Simulation to execute)")
    else:
        result["execution"] = ("plan only: no person named; add "
                               "'and prepare it for <name>' to execute")
    return result


def ask_world(world: RealityWorld, text: str, runtime=None,
    ...
    if m:
        ...
        return {"type": "entity_status", "query": q,
                "entity": _entity_full(world, entity),
                "recent_changes": changes[-5:]}

    m = re.match(r"why is (.+?) (unavailable|reserved|occupied|failed|closed)\??$",
                 low)
    if m:
        entity = _find_entity_w(world, m.group(1))
        if entity is None:
            return {"type": "not_found", "query": q,
                    "message": f"no entity matching {m.group(1).strip()!r}"}
        word = m.group(2)
        matching = [a for a, v in entity.state.items()
                    if str(v).lower() == word or a == "availability"]
        return {"type": "state_explanation", "query": q,
                "entity": _entity_full(world, entity),
    ...
    if m:
        ...
        return {"type": "changes", "query": q, "since": since,
                "count": len(events), "events": events[-10:]}
    ...
    if m:
        ...
        if entity is None:
            return {"type": "not_found", "query": q,
                    "message": f"no entity matching {m.group(1).strip()!r}"}
        return {"type": "history", "query": q, "entity_id": entity.id,
                "transitions": world.history(entity.id)}

    m = re.match(
        r"what evidence supports (.+?) being in (.+?)\??$", low)
    if m:
        entity = _find_entity_w(world, m.group(1))
        if entity is None:
            return {"type": "not_found", "query": q,
                    "message": f"no entity matching {m.group(1).strip()!r}"}
        zid = _find_zone_id(
            SpatialPipeline(world.spatial), m.group(2))
    ...
    if m:
        ...
        zone_id = None
        if m.group(2):
            zone_id = _find_zone_id(SpatialPipeline(world.spatial),
                                    m.group(2))
        return {"type": "capabilities", "query": q,
    ...
    if m:
        ...
        if runtime is None:
            return {"type": "action_plan", "query": q,
                    "plan": [{"action": "move_entity", "target": entity.id,
                              "parameters": {"to_zone": zid}}],
                    "note": "no runtime attached; nothing was executed"}
        try:
            action = runtime.act(
                "move_entity", actor="ask", target=entity.id,
                parameters={"to_zone": zid}, provenance="ask",
                approved_by=approved_by)
        except ValueError as exc:
            return {"type": "action_rejected", "query": q,
                    "reason": str(exc)}
        return {"type": "action_result", "query": q,
                "action": action.to_dict()}

    m = re.match(r"(?:did|verify) (.+?) (succeed|fail)\??$", low) \
        or re.match(r"verify (.+?)\??$", low)
    if m:
        aid = m.group(1).strip()
        action = world.actions.get(aid)
        if action is None:
            return {"type": "not_found", "query": q,
                    "message": f"no action {aid!r}"}
        return {"type": "action_status", "query": q,
                "action": action.to_dict()}

    m = re.match(
        r"find (?:an )?available workstation near (.+?)"
        r"(?: and prepare (?:it )?for (.+?))?\??$", low)
    if m:
        return _plan_workstation(world, m.group(1).strip(),
